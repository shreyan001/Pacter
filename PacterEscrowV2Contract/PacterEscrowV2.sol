...
contract PacterEscrowV2 {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "Pacter: Caller is not the owner");
        _;
    }


    modifier onlyVerificationAgent() {
        require(msg.sender == verificationAgent, "Pacter: Caller is not the verification agent");
        _;
    }


    modifier onlyInitiator(bytes32 orderHash) {
        require(msg.sender == orders[orderHash].initiator, "Pacter: Caller is not the initiator");
        _;
    }


    modifier onlyFreelancer(bytes32 orderHash) {
        require(msg.sender == orders[orderHash].freelancer, "Pacter: Caller is not the freelancer");
        _;
    }
    
    modifier validateOrderExists(bytes32 orderHash) {
        require(orders[orderHash].createdTimestamp != 0, "Pacter: Order does not exist");
        _;
    }
    ...
    function createAndDeposit(
        ...
        require(orderHash != 0, "Pacter: Invalid order hash");
        require(orders[orderHash].createdTimestamp == 0, "Pacter: Order already exists");
        require(freelancer != address(0), "Pacter: Invalid freelancer address");


        uint256 totalDeposit = escrowAmount + storageFee;
        require(msg.value == totalDeposit, "Pacter: Incorrect 0G token amount sent");
    ...
    function verifyDeliverable(
        ...
        require(order.currentState == OrderState.ACTIVE, "Pacter: Order not in active state for verification");
    ...
    function approvePayment(bytes32 orderHash) external validateOrderExists(orderHash) onlyInitiator(orderHash) {
        Order storage order = orders[orderHash];
        require(order.currentState == OrderState.VERIFIED, "Pacter: Order must be verified before approval");
    ...
    function withdrawFunds(bytes32 orderHash) external validateOrderExists(orderHash) onlyFreelancer(orderHash) {
        Order storage order = orders[orderHash];
        require(order.currentState == OrderState.APPROVED, "Pacter: Payment not approved for withdrawal");
        ...
        require(success, "Pacter: 0G token transfer to freelancer failed");
    ...
    function transfer0GStorageFees(uint256 amount) external onlyOwner {
        require(amount > 0, "Pacter: Amount must be greater than zero");
        require(address(this).balance >= amount, "Pacter: Insufficient balance for fee transfer");


        (bool success, ) = payable(zeroGServiceWallet).call{value: amount}("");
        require(success, "Pacter: 0G storage fee transfer failed");
    ...
    function updateVerificationAgent(address newAgent) external onlyOwner {
        require(newAgent != address(0), "Pacter: Invalid agent address");
    ...
    function updateServiceWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Pacter: Invalid wallet address");
    ...
    receive() external payable {
        revert("Pacter: Use createAndDeposit function");
    }
