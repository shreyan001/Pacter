...
export default function ContractDisplay({ contract }: ContractDisplayProps) {
    ...
    // Debug logging
    console.log('ContractDisplay - Contract data:', contract)
    console.log('ContractDisplay - Contract parties:', contract.parties)
    console.log('ContractDisplay - Contract escrow:', contract.escrow)
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="p-2 bg-slate-800/50 rounded border border-slate-600/20">
                        ...
                        <div className="space-y-0.5 text-slate-400">
                            <p className="text-slate-300">{contract.parties?.client?.name || 'Unknown'}</p>
                            <p className="text-slate-400">{contract.parties?.client?.email || 'No email'}</p>
                            <p className="text-[10px] break-all text-slate-500">{contract.parties?.client?.walletAddress || 'No wallet address'}</p>
            ...
            <div className="bg-slate-700/80 rounded-lg p-3 border border-slate-600/30">
                ...
                <div className="space-y-2 text-slate-400">
                    <div className="flex justify-between">
                        ...
                        <span className="text-slate-200 font-semibold">₹{contract.escrow?.amounts?.inr?.totalAmount || '0'}</span>
                    </div>
                    <div className="flex justify-between">
                        <span>Total Amount (0G):</span>
                        <span className="text-slate-200 font-semibold">{contract.escrow?.amounts?.['0G']?.totalAmount || '0'} 0G</span>
                    </div>
                    <div className="pt-2 border-t border-slate-600/30">
                        ...
                        <div className="space-y-1 pl-2">
                            <div className="flex justify-between">
                                ...
                                <span>₹{contract.escrow?.fees?.platformFee?.inr || '0'}</span>
                            </div>
                            <div className="flex justify-between">
                                <span>Storage Fee:</span>
                                <span>₹{contract.escrow?.fees?.storageFee?.inr || '0'}</span>
                            </div>
                            <div className="flex justify-between font-semibold text-slate-300 pt-1 border-t border-slate-600/20">
                                <span>Total Fees:</span>
                                <span>₹{contract.escrow?.fees?.totalFees?.inr || '0'}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-slate-700/80 rounded-lg p-3 border border-slate-600/30">
                ...
                <div className="space-y-2 text-slate-400">
                    <div>
                        ...
                        <span className="text-slate-300 ml-1">{contract.projectDetails?.timeline || 'Not specified'}</span>
                    </div>
                    <div className="flex gap-4">
                        <div className="flex items-center gap-1">
                            <Calendar className="w-3 h-3 text-slate-500" />
                            <span className="text-slate-500">Start:</span>
                            <span className="text-slate-300">{contract.projectDetails?.startDate ? new Date(contract.projectDetails.startDate).toLocaleDateString() : 'Not specified'}</span>
                        </div>
                        <div className="flex items-center gap-1">
                            <Calendar className="w-3 h-3 text-slate-500" />
                            <span className="text-slate-500">End:</span>
                            <span className="text-slate-300">{contract.projectDetails?.endDate ? new Date(contract.projectDetails.endDate).toLocaleDateString() : 'Not specified'}</span>
                        </div>
                    </div>
                    <div>
                        ...
                        <ul className="list-disc list-inside space-y-0.5 pl-2">
                            ...
                            )) || <li className="text-slate-300">No deliverables specified</li>}
                        </ul>
                    </div>
                </div>
            </div>
