...
export function SmartContractDisplay({ contractCode }: { contractCode: string }) {
  ...
  const getContractType = (contractName: string): string => {
    ...
    return 'Autonomous Escrow'
  }
  
  const getContractDescription = (contractName: string): string => {
    if (contractName.includes('NFT')) return 'Autonomous escrow for digital asset and NFT exchanges'
    if (contractName.includes('ERC20')) return 'Programmable trust for token-to-token exchanges'
    if (contractName.includes('Asset')) return 'AI-verified escrow for digital asset transactions'
    return 'Autonomous middleman protocol for secure agreements'
  }
