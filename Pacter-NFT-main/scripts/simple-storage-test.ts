...
async function main() {
  ...
  try {
    ...
    while (attempts < maxAttempts && !fileAvailable) {
      try {
        const fileInfo = await indexer.getFileInfo(tree.rootHash());
        if (fileInfo && fileInfo.finalized) {
          fileAvailable = true;
          console.log(`✅ File is available and finalized in storage!`);
          console.log(`📊 File Info:`, {
            size: fileInfo.tx.size,
            finalized: fileInfo.finalized
          });
        } else {
          attempts++;
          console.log(`⏳ Attempt ${attempts}/${maxAttempts} - File not yet finalized, waiting...`);
    ...
    const downloadErr = await indexer.download(tree.rootHash(), downloadPath, true);
